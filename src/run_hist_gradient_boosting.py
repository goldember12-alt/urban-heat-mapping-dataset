...
from src.modeling_runner import run_hist_gradient_boosting_model
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run grouped histogram-gradient-boosting hotspot modeling on the canonical parquet-first dataset "
            "as the bounded Phase 1 better-learner checkpoint."
        ),
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional explicit output directory. When omitted, the CLI generates a unique run directory under outputs/modeling/hist_gradient_boosting/.",
    )
    ...
    parser.add_argument(
        "--max-iter",
        type=int,
        default=DEFAULT_HIST_GRADIENT_BOOSTING_MAX_ITER,
        help="Maximum boosting iterations for each histogram-gradient-boosting fit.",
    )
    parser.add_argument(
        "--grid-search-n-jobs",
        type=int,
    ...
    parser.add_argument(
        "--tuning-preset",
        choices=get_valid_tuning_presets("hist_gradient_boosting"),
        default=DEFAULT_TUNING_PRESET,
        help=get_tuning_preset_help_text("hist_gradient_boosting"),
    )
    return parser
