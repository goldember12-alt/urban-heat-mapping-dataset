...
def build_presentation_assets(repo_root: Path, output_dir: Path) -> list[Path]:
    ...
    slide_builders = [
        ...
        ("slide_02_problem_framing.png", lambda: _build_problem_slide()),
...
def _build_title_slide(data: PresentationData) -> Image.Image:
    ...
    _draw_vertical_glow(image, (1110, -30), 560, "#e9976d33")
    _draw_vertical_glow(image, (1360, 250), 360, "#4e879433")
    _draw_vertical_glow(image, (320, 720), 260, "#efd9b033")

    kicker_font = _font(36, bold=True)
    hero_font = _font(74, bold=True)
    sub_font = _font(36)
    footer_font = _font(30)
    stat_font = _font(31, bold=True)

    draw.rounded_rectangle((62, 58, 450, 122), radius=20, fill=ACCENT)
    draw.text((96, 74), "Held-out-city transfer", font=kicker_font, fill=WHITE)

    hero_text = "30 cities | 71.4M cells | hotspot screening on unseen cities"
    _draw_wrapped(draw, hero_text, (64, 148), 950, hero_font, INK, line_spacing=16)
    _draw_wrapped(
        draw,
    ...
    draw.rounded_rectangle((1010, 110, 1530, 710), radius=30, fill="#fff8ef")
    draw.rounded_rectangle((1048, 156, 1492, 234), radius=22, fill=TEAL)
    draw.text((1092, 176), "What the talk shows", font=_font(34, bold=True), fill=WHITE)
    ...
    for bullet in bullets:
        draw.ellipse((1070, y + 10, 1090, y + 30), fill=ACCENT)
        _draw_wrapped(draw, bullet, (1120, y), 330, stat_font, INK, line_spacing=8)
        y += 106

    draw.rounded_rectangle((62, 794, 1530, 864), radius=24, fill="#ebe0cf")
    footer = "Max Clements   |   STAT 5630 Final Project   |   April 2026"
    draw.text((96, 816), footer, font=footer_font, fill=TEAL_DARK)
    return image


def _build_problem_slide() -> Image.Image:
    image = _base_canvas()
    draw = ImageDraw.Draw(image)
    _draw_vertical_glow(image, (1290, 110), 300, "#d26b3422")
    _draw_vertical_glow(image, (180, 720), 240, "#3a718222")
    ...
    x_positions = [54, 548, 1042]
    for x, (headline, subline) in zip(x_positions, card_specs):
        draw.rounded_rectangle((x, 54, x + 446, 206), radius=26, fill="#fffaf2", outline="#dfc7a4", width=2)
        draw.text((x + 28, 84), headline, font=_font(34, bold=True), fill=INK)
        _draw_wrapped(draw, subline, (x + 28, 134), 388, _font(27), MUTED)

    draw.rounded_rectangle((60, 248, 1540, 618), radius=40, fill=WHITE, outline="#d7c2a0", width=3)
    draw.rounded_rectangle((108, 292, 422, 356), radius=20, fill=ACCENT_DARK)
    draw.text((142, 308), "Core question", font=_font(34, bold=True), fill=WHITE)
    question = "Can a model trained on some cities find heat hotspots in a city it has never seen?"
    _draw_wrapped(draw, question, (114, 406), 1310, _font(62, bold=True), INK, line_spacing=16)

    draw.rounded_rectangle((186, 716, 1414, 804), radius=28, fill="#dfe8e8")
    caption = "This is a transfer benchmark, not same-city interpolation."
    w = _text_bbox(draw, caption, _font(40, bold=True))[2]
    draw.text(((1600 - w) / 2, 737), caption, font=_font(40, bold=True), fill=TEAL_DARK)
    return image
...
def _build_design_slide(data: PresentationData) -> Image.Image:
    image = _base_canvas()
    draw = ImageDraw.Draw(image)
    _draw_vertical_glow(image, (1370, 770), 220, "#3a71821e")
    ...
    for value, label in stat_cards:
        draw.rounded_rectangle((x, 42, x + 300, 222), radius=26, fill="#fffaf2", outline="#dbc7aa", width=2)
        value_font_size = 54
        if len(value) >= 11:
            value_font_size = 34
        elif len(value) >= 10:
            ...
            value_font_size = 46
        draw.text((x + 26, 80), value, font=_font(value_font_size, bold=True), fill=INK)
        draw.text((x + 26, 156), label, font=_font(30), fill=MUTED)
        x += 314

    draw.rounded_rectangle((1308, 42, 1548, 222), radius=26, fill=ACCENT)
    draw.text((1370, 80), f"{data.outer_fold_count}", font=_font(56, bold=True), fill=WHITE)
    draw.text((1352, 156), "outer folds", font=_font(32), fill=WHITE)

    draw.rounded_rectangle((50, 274, 992, 812), radius=30, fill=WHITE, outline="#d8c6ab", width=2)
    draw.rounded_rectangle((84, 308, 386, 376), radius=20, fill=TEAL)
    draw.text((120, 324), "Analytic unit", font=_font(33, bold=True), fill=WHITE)
    _draw_wrapped(
        draw,
    ...
    draw.rounded_rectangle((1020, 274, 1548, 812), radius=30, fill="#fffaf2", outline="#d8c6ab", width=2)
    draw.rounded_rectangle((1044, 308, 1524, 376), radius=20, fill=ACCENT_DARK)
    draw.text((1074, 326), "Leakage-safe benchmark", font=_font(30, bold=True), fill=WHITE)
    _draw_wrapped(
        draw,
    ...
    for index, (label, fill) in enumerate(pipeline_boxes):
        box_w = 212 if index == 0 else 246 if index == 3 else 228
        draw.rounded_rectangle((x, pipeline_y, x + box_w, pipeline_y + 106), radius=24, fill=fill)
        _draw_wrapped(draw, label, (x + 20, pipeline_y + 16), box_w - 40, _font(26, bold=True), WHITE)
        if index < len(pipeline_boxes) - 1:
            _draw_arrow(draw, (x + box_w + 18, pipeline_y + 52), (x + box_w + 64, pipeline_y + 52), fill=MUTED, width=10)
        x += box_w + 82
...
def _build_models_results_slide(data: PresentationData) -> Image.Image:
    ...
    draw.rounded_rectangle((34, 34, 486, 844), radius=30, fill="#fffaf2", outline="#d8c5a7", width=2)
    draw.rounded_rectangle((58, 68, 462, 126), radius=18, fill=TEAL)
    draw.text((88, 80), "Same predictor contract", font=_font(24, bold=True), fill=WHITE)
    _draw_wrapped(
        draw,
    ...
    model_cards = [
        ("Logistic regression", "Pr(Y=1|x) = sigma(beta_0 + x^T beta)", "Linear additive baseline"),
        ("Random forest", "p_hat(x) = (1/B) sum T_b(x)", "Tree ensemble for nonlinear effects"),
    ]
    y = 254
    for title, equation, interpretation in model_cards:
        draw.rounded_rectangle((66, y, 454, y + 208), radius=24, fill=WHITE, outline="#d9d9d1", width=2)
        draw.text((92, y + 24), title, font=_font(30, bold=True), fill=INK)
        _draw_wrapped(draw, equation, (92, y + 84), 316, _font(23, bold=True), ACCENT_DARK, line_spacing=5)
        _draw_wrapped(draw, interpretation, (92, y + 164), 318, _font(22), MUTED)
        y += 222

    benchmark = Image.open(data.benchmark_figure_path).convert("RGB")
    benchmark = ImageOps.contain(benchmark, (1040, 472))
    image.paste(benchmark, (510, 24))
    ...
    x_positions = [530, 878, 1226]
    for (text, fill, text_fill), x in zip(metrics, x_positions):
        draw.rounded_rectangle((x, 538, x + 312, 706), radius=26, fill=fill)
        _draw_wrapped(draw, text, (x + 26, 574), 258, _font(32, bold=True), text_fill, line_spacing=10)

    caveat = "Matched sampled all-fold checkpoints; not exhaustive 71M-cell scoring."
    draw.rounded_rectangle((526, 748, 1534, 814), radius=20, fill="#ebe2d5")
    draw.text((562, 768), caveat, font=_font(27), fill=MUTED)
    return image
...
def _build_denver_slide(data: PresentationData) -> Image.Image:
    ...
    denver = ImageOps.contain(denver, (1528, 686))
    image.paste(denver, (36, 40))

    draw.rounded_rectangle((1130, 780, 1544, 850), radius=22, fill=TEAL)
    draw.text((1170, 802), "RF frontier | hot-arid", font=_font(30, bold=True), fill=WHITE)

    draw.rounded_rectangle((38, 772, 1088, 856), radius=24, fill="#f0e4d4")
    caption = "Prediction errors remain spatially structured rather than random."
    _draw_wrapped(draw, caption, (70, 794), 944, _font(38, bold=True), INK, line_spacing=8)
    return image
...
def _build_takeaway_slide(data: PresentationData) -> Image.Image:
    image = _base_canvas()
    draw = ImageDraw.Draw(image)
    _draw_vertical_glow(image, (1370, 120), 280, "#d26b3433")
    _draw_vertical_glow(image, (280, 738), 240, "#3a718222")

    draw.rounded_rectangle((48, 48, 1552, 304), radius=36, fill=WHITE, outline="#d8c6ab", width=2)
    takeaway = "Yes: cross-city hotspot transfer is feasible, but performance is not yet uniform across climates."
    _draw_wrapped(draw, takeaway, (86, 102), 1388, _font(64, bold=True), INK, line_spacing=18)
    ...
    for heading, body, fill in items:
        ...
        draw.rounded_rectangle((x, 382, x + 480, 732), radius=30, fill=fill)
        draw.text((x + 30, 424), heading, font=_font(34, bold=True), fill=text_fill)
        draw.rounded_rectangle((x + 24, 502, x + 456, 686), radius=24, fill=body_fill)
        _draw_wrapped(draw, body, (x + 44, 532), 384, _font(31), INK, line_spacing=10)
        x += 512

    draw.text((1290, 820), "Questions?", font=_font(34, bold=True), fill=MUTED)
    return image
