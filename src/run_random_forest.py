...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run grouped random-forest hotspot modeling on the canonical parquet-first dataset.",
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="RandomForest worker count. Keep the default for normal runs; constrained sandboxes may still prefer 1 for smoke verification.",
    )
    parser.add_argument(
        "--tuning-preset",
        choices=get_valid_tuning_presets("random_forest"),
        default=DEFAULT_TUNING_PRESET,
        help=get_tuning_preset_help_text("random_forest"),
    )
    return parser
